<div class="bg-gradient-to-bl from-quicksilver-600 to-quicksilver-700 text-quicksilver-100 py-24 px-4 xl:px-0">
    <div class="sm:w-full sm:max-w-xl mx-auto">
        <h2 class="mt-0 text-3xl text-bigWaves-50">How to Finance Your Real Estate Investment</h2>
        <p class="text-2xl">
            Our process is simple! We handle the entire process of the loan. Loans are granted in <em>as little as 21
                days</em>.
        </p>
    </div>
    <div class="mx-auto grid grid-cols-1 gap-8 md:grid-cols-5 sm:max-w-xl md:max-w-6xl">
        <div>
            <h3 class="text-quicksilver-100">1. Get Started</h3>
            <p>
                <a href="/application" class="text-quicksilver-100 hover:text-quicksilver-50">Fill in the form</a> or
                call us at
                856-422-3232 so we can
                contact you and begin working on your loan. We're local and happy to meet with
                you to learn your funding needs.
            </p>
        </div>

        <div>
            <h3 class="text-quicksilver-100">2. Pre-qualify</h3>
            <p>Complete your application and provide supporting documentation.</p>
        </div>

        <div>
            <h3 class="text-quicksilver-100">3. Proof of Funds</h3>
            <p>Increase your buying power with a proof of funds (PDF) letter.</p>
        </div>
        <div>
            <h3 class="text-quicksilver-100">4. Loan Processing</h3>
            <p>
                Your appraisal and title work are ordered. Your application documents are
                reviewed.
            </p>
        </div>
        <div>
            <h3 class="text-quicksilver-100">5. Get Funded Now</h3>
            <p>Submit project information for quick, in-house review.</p>
        </div>
    </div>
</div>
