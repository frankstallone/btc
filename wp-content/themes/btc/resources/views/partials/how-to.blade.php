<div class="bg-emeraldCity-50 p-12">
    <div class="sm:w-full sm:max-w-2xl mx-auto px-4 sm:px-0">
        <h2 class="mt-0 text-emeraldCity-800 text-3xl text-center">How to Finance Your Real Estate Investment</h2>
        <p class="mb-0 text-quicksilver-900">
            Our process is simple! We handle the entire process of the loan. Loans are granted in <em>as little as 21
                days</em>.
        </p>
    </div>
    <div class="mx-auto grid grid-cols-1 gap-8 px-4 md:grid-cols-5 md:max-w-6xl md:px-0">
        <div>
            <h3 class="text-emeraldCity-800">1. Get Started</h3>
            <p class="text-quicksilver-900">
                <a href="#">Fill in the form</a> or call us at 856-422-3232 so we can
                contact you and begin working on your loan. We're local and happy to meet with
                you to learn your funding needs.
            </p>
        </div>

        <div>
            <h3 class="text-emeraldCity-800">2. Pre-qualify</h3>
            <p class="text-quicksilver-900">Complete your application and provide supporting documentation.</p>
        </div>

        <div>
            <h3 class="text-emeraldCity-800">3. Proof of Funds</h3>
            <p class="text-quicksilver-900">Increase your buying power with a proof of funds (PDF) letter.</p>
        </div>
        <div>
            <h3 class="text-emeraldCity-800">4. Loan Processing</h3>
            <p class="text-quicksilver-900">
                Your appraisal and title work are ordered. Your application documents are
                reviewed.
            </p>
        </div>
        <div>
            <h3 class="text-emeraldCity-800">5. Get Funded Now</h3>
            <p class="text-quicksilver-900">Submit project information for quick, in-house review.</p>
        </div>
    </div>
</div>
